/*
	curl-to-jsoup
	By ublack
	Based on curl-to-java by fivesmallq https://github.com/ublack/curl-to-jsoup
	A simple utility to convert curl commands into Java code(Jsoup).
*/

function curlToJsoup(curl) {
	var statusPrint = 'Document doc = conn.ignoreContentType(true).execute().parse();\nSystem.out.println(doc);\n';
	var promo = "// Generated by curl-to-jsoup: https://ublack.github.io/curl-to-jsoup";
	...
	function renderSimple(method, url) {
		if (method == "Get")
			return 'Connection conn = Jsoup.connect("' + url + '").method(Connection.Method.GET);\n' + statusPrint ;
		else if (method == "Post")
			return 'Connection conn = Jsoup.connect("' + url + '").method(Connection.Method.POST);\n' + statusPrint ;
		else if (method == "Head")
			return 'Connection conn = Jsoup.connect("' + url + '").method(Connection.Method.HEAD);\n' + statusPrint ;
		else
			return 'Connection conn = Jsoup.connect("' + url + '").method(Connection.Method.XXX);\n' + statusPrint ;
	}
	...
	function renderComplex(req) {
		...
		if (!req.data.ascii && !req.data.files) {
			...
			java += 'Connection conn = Jsoup.connect("' + req.url + '").method(Connection.Method.' + req.method.toUpperCase() + ');\n';
		} else {
			var ioReaders = [];
			java += 'Connection conn = Jsoup.connect("' + req.url + '").method(Connection.Method.' + req.method.toUpperCase() + ');\n';
			...
			if (req.data.files && req.data.files.length > 0) {
				...
				for (var i = 0; i < req.data.files.length; i++) {
					...
					java += "FileInputStream " + thisVarName + '= new FileInputStream("' + req.data.files[i] + '");\n';
					java += 'conn.data("' + thisVarName + '", "' + thisVarName + '", f);\n';
		...
		for (var name in req.headers) {
			java += 'conn.header("'+name+'", "'+req.headers[name]+'");\n';
		}
